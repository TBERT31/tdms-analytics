...
export class CircuitBreaker extends EventEmitter {
  ...
  private lastError: Error | undefined = undefined; // The last error encountered
  private nextAttempt: number = 0; // Timestamp for the next attempt when in Open state
  private readonly options: CircuitBreakerOptions; // Configuration options
  ...
  constructor(
    ...
    this.options = {
      successThreshold:
        ...
        this.configService.get<number>('CIRCUIT_BREAKER_SUCCESS_THRESHOLD', 3),
      failureThreshold:
        options.failureThreshold ??
        this.configService.get<number>('CIRCUIT_BREAKER_FAILURE_THRESHOLD', 3),
      openToHalfOpenWaitTime:
        options.openToHalfOpenWaitTime ??
  ...
  exec(next: CallHandler) {
    ...
    return next.handle().pipe(
      tap({
        ...
        error: (error) => this.handleError(error as Error), // Call handleError on error
      }),
    );
  }
